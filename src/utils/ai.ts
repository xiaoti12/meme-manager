...
export class AIVisionService {
  ...
  /**
   * 根据图片内容生成标签
   * @param description AI生成的描述
   * @returns 标签数组
   */
  static generateTags(description: string): string[] {
    const tags: string[] = []
    const lowerDesc = description.toLowerCase()

    // 情感标签
    if (lowerDesc.includes('开心') || lowerDesc.includes('笑') || lowerDesc.includes('快乐')) {
      tags.push('开心', '快乐')
    }
    if (lowerDesc.includes('伤心') || lowerDesc.includes('哭') || lowerDesc.includes('泪')) {
      tags.push('伤心', '哭泣')
    }
    if (lowerDesc.includes('惊讶') || lowerDesc.includes('震惊')) {
      tags.push('惊讶', '震惊')
    }
    if (lowerDesc.includes('愤怒') || lowerDesc.includes('生气')) {
      tags.push('愤怒', '生气')
    }

    // 风格标签
    if (lowerDesc.includes('动漫') || lowerDesc.includes('二次元') || lowerDesc.includes('anime')) {
      tags.push('动漫', '二次元')
    }
    if (lowerDesc.includes('卡通') || lowerDesc.includes('cartoon')) {
      tags.push('卡通')
    }
    if (lowerDesc.includes('可爱') || lowerDesc.includes('萌')) {
      tags.push('可爱', '萌')
    }

    // 角色标签
    if (lowerDesc.includes('猫') || lowerDesc.includes('cat')) {
      tags.push('猫', '猫咪')
    }
    if (lowerDesc.includes('狗') || lowerDesc.includes('dog')) {
      tags.push('狗', '狗狗')
    }
    if (lowerDesc.includes('少女') || lowerDesc.includes('女孩')) {
      tags.push('少女', '女孩')
    }
